...
import { addToCache, getFromCache ,deleteFromCache} from "../middleware/cache.js";
...
export default {
	...
	login: async (req, res) => {
		...
		deleteFromCache([`refreshToken_${myInfo.id}`,`refreshTokenNotValid_${myInfo.id}`]);
		...
		res.status(StatusCodes.OK).json({
			...
			data: {
				...
				refreshToken,
				role: myInfo.role,
				verification_email: myInfo.verification_email === "0" ? false : true,
			},
		});
	},
	/*
	...
	refreshToken: async (req, res) => {
		...
		if (!myInfo) throw new Error(req.getLocalLanguage("error.users.not_found"));
		let deviceId = uuidv4();
		let token = generateToken(
			{
				userId: myInfo.id,
