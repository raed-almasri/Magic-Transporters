...
import { ITripWithMagicMover } from "../../typescript/index.js";
export default {
    create: async (req: Request, res: Response) => {
        ...
        let weight_all: { sum: number } | any = await MagicItems.findAll({
            raw: true,
        ...
        if (
            weight_all.length === 0 ||
            (req.body.weight <=
                check.magic_mover_info.weight - weight_all[0].sum &&
    ...
    remove: async (req: Request, res: Response) => {
        let check: ITripWithMagicMover | any = await MagicItems.findOne({
            raw: true,
            attributes: ["id"],
            nest: true,
